...
export class ListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.service.currentSearchText.subscribe(searchText => {
      this.searchCustomersList = this.customers.filter(x => 
        (new RegExp(searchText).test(x.name) 
        || new RegExp(searchText).test(x.location)
        ))
    })
  }
